...
| (10) 평가 ||
| (11) 결론 |Truth Tag는 식료품 물가 인상 상황에서 간편하게 구매 판단을 도와드리고자 만들었습니다. 슈링크/스킴플레이션 등의 물가 인상 전략으로 인해 발생하는 문제점은 개인이 극복하기에 어렵다고 판단했기 때문입니다. 더불어, 참가격, 소비자 24, 다나와, 에누리 등의 기존 서비스들이 제공하고 있는 기능에는 가시성, 단순성, 간편성 등의 측면에서 부족함이 있다는 것을 발견하기도 했습니다. <br><br> 이와 같은 한계를 극복하기 위해 Truth Tag는 네 가지 주요 기능을 제공합니다. 첫번째로, 식료품별 대시보드에서 용량 추이 그래프와 원재료 변동 테이블을 통해, 정보를 한눈에 파악할 수 있게 합니다. 두번째로, 식료품 구매 도우미 챗봇을 통해 5가지 기능을 단순한 메세지로 사용하고 어떤 식료품을 구매할지 간단하게 결정하는 것에 도움을 받을 수 있습니다. 세번째로, 상황에 맞게 식료품을 검색할 수 있도록 상품명 검색, 원재료 검색, 이미지 검색 기능을 갖추고 있습니다. 마지막으로, 로그인한 사용자라면 관심있는 식료품을 즐겨찾기로 등록해, 해당 식료품의 용량이나 원재료가 변경되었을 때 알림을 받을 수 있습니다. <br><br> Truth Tag는 기술적인 성능을 높여 뛰어난 사용자 경험을 제공합니다. 용량 추이 그래프는 Nivo Chart 라이브러리를 사용해 로딩 속도를 xx% 줄였습니다. 또한, 이미지 검색에 사용한 OCR 모델은 EasyOCR을, 챗봇 모델은 KoBERT 모델을 finetuning하여 정확도를 각각 xx%, xx%로 높였습니다. 아울러, API 호출 시 JPA와 QueryDSL을 사용하여 최대 xx초의 속도 개선을 했습니다. <br><br> 결과적으로, Truth Tag를 통해 소비자들이 현명한 소비 문화를 형성하고, 기업의 불공정한 전략을 억제하기를 기대합니다.|
| (12) 기타 |  |
...
| (7) 과제 내용 요약 | Psylog는 OpenAI의 fine tuning 기술을 활용해 사용자의 일기를 깊이 이해합니다. 단순히 글을 읽는 것을 넘어, 사용자의 감정을 세심하게 분석하고, 개인 맞춤형 공감과 조언을 제공합니다. 또한, 하루 동안의 감정 분포를 시각적으로 나타내는 그래프를 통해 자신의 심리 상태를 한눈에 파악할 수 있도록 도와줍니다. 이를 통해 사용자는 자신의 감정 변화를 이해하고, 긍정적인 정신 건강을 유지하는 데 필요한 지원을 받을 수 있습니다. |
| (8-1) 과제: 문제의 정의 | 많은 사람들이 사회적 압력, 편견, 처한 환경 때문에 자신의 감정을 억누르거나 숨기게 된다. 감정을 솔직히 털어 놓지 못하는 것은 스트레스, 정신 건강 문제로 이어지게 된다. <br>이러한 문제점을 해결하고자 자신의 속마음을 솔직히 털어놓고 공감 및 조언을 받고 싶은 현대인들을 타겟 사용자로 하여 PSYLog 사용자들이 타인에게 쉽게 털어놓을 수 없는 이야기를 해당 서비스를 통해 털어놓고 이에 대한 공감 및 조언을 제공하고자 한다. |
| (8-2) 과제: 기존연구와의 비교 | 기존의 다이어리 서비스들은 단순히 사용자가 작성한 일기를 저장하고 추후에 이를 다시 볼 수 있는 기능을 제공한다. 이런 기존 서비스들과 달리 PSYLog는 사용자의 일기장 속 친구가 되어 사용자가 작성한 일기를 저장할 뿐만 아니라 이를 분석하여 사용자의 하루에 공감해주며 도움이 될 수 있는 조언을 제공한다. <br>그리고 기존의 감정 문제 해결을 위한 대화형 챗봇 서비스들과 달리 PSYLog는 긴 텍스트를 전체적으로 분석하여 공감과 조언을 제공하고자 한다.  |
| (8-3) 과제: 제안 내용 | 1. OpenAi fine tuning을 prompting하여 사용자의 일기를 input으로 받아와 이를  토대로 8~9줄 정도의 공감,조언을 포함한 메시지를 전달한다. <br>2. Clover sentiment를 이용하여 사용자의 일기를 분석하고 해당 결과를 토대로  감정 분석 그래프를 도출(6가지 감정의 분포도)한다.  |
| (8-4) 과제: 과제의 주요 기능 | - 일기 작성 및 수정, 일기 모아보기 (기본 다이어리 기능) <br>- 공감 : 일기를 분석하여 친한 친구처럼 사용자의 하루에 공감  <br>- 조언 : 사용자의 상황에 맞는 맞춤형 조언 제공  <br>- 가시적 분석 : 일기를 기반으로 감정 분석 그래프 제공 (6가지 감정의 분포도)   |
| (8-5) 과제: 구현 방법 | |
| (8-6) 과제: 세부 기술 | |
| (8-7) 과제: 기대 효과 및 의의 | |
| (9) 데모 내용 |  . |
